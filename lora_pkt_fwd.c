...
static char serv_url[64] = STR(DEFAULT_SERVER); /* address of the server (host name or IPv4/IPv6) */
...
static int parse_gateway_configuration(const char * conf_file) {
    ...
    /* server url (optional) */
    str = json_object_get_string(conf_obj, "server_url");
    if (str != NULL) {
        strncpy(serv_url, str, sizeof serv_url);
        serv_url[sizeof serv_url - 1] = '\0'; /* ensure string termination */
        MSG("INFO: server url address is configured to \"%s\"\n", serv_url);
    }
...
void thread_up(void) {
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
	    ...
	    printf("\nJSON stat: %s\n", (char *)(buff_up + 11)); /* DEBUG: display JSON payload */ 	
	    webhook(serv_url,(char *)(buff_up + 11)); 	
	} else {
